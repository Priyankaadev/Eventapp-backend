...
const messageSchemaKey = require('../../../utils/validation/messageValidation');
...
/**
 * @description : create document of Message in mongodb collection.
 * @param {Object} req : request including body for creating document.
 * @param {Object} res : response of created document
 * @return {Object} : created Message. {status, message, data}
 */ 
const addMessage = async (req, res) => {
  try {
    ...
    dataToCreate = new Message(dataToCreate);
    let createdMessage = await dbService.create(Message,dataToCreate);
    return res.success({ data : createdMessage });
  } catch (error) {
 ...
 */
const bulkInsertMessage = async (req,res)=>{
    try {
      ...
      let createdMessages = await dbService.create(Message,dataToCreate);
      createdMessages = { count: createdMessages ? createdMessages.length : 0 };
      return res.success({ data:{ count:createdMessages.count || 0 } });
    } catch (error){
      return res.internalServerError({ message:error.message });
    }
  };
...
const findAllMessage = async (req,res) => {
    try {
      ...
      let validateRequest = validation.validateFilterWithJoi(
        req.body,
        messageSchemaKey.findFilterKeys,
        Message.schema.obj
      );
      if (!validateRequest.isValid) {
      ...
      let foundMessages = await dbService.paginate( Message,query,options);
      if (!foundMessages || !foundMessages.data || !foundMessages.data.length){
...
const getMessage = async (req,res) => {
    try {
      ...
      let options = {};
      let foundMessage = await dbService.findOne(Message,query, options);
      if (!foundMessage){
        return res.recordNotFound();
      }
      return res.success({ data :foundMessage });
    }
    catch (error){
      return res.internalServerError({ message:error.message });
    }
...
const getMessageCount = async (req,res) => {
    try {
      ...
      let countedMessage = await dbService.count(Message,where);
      return res.success({ data : { count: countedMessage } });
    } catch (error){
      return res.internalServerError({ message:error.message });
    }
  };
...
const updateMessage = async (req,res) => {
    ...
    // let {name, city, date, status} = req.body;
      ...
      let updatedMessage = await dbService.updateOne(Message,query,dataToUpdate);
      if (!updatedMessage){
        return res.recordNotFound();
      }
 ...
 */
const bulkUpdateMessage = async (req,res)=>{
    try {
      ...
      let updatedMessage = await dbService.updateMany(Message,filter,dataToUpdate);
      if (!updatedMessage){
        return res.recordNotFound();
      }
      return res.success({ data :{ count : updatedMessage } });
    } catch (error){
      return res.internalServerError({ message:error.message }); 
    }
  };
...
const softDeleteMessage = async (req,res) => {
    try {
      ...
      let updatedMessage = await dbService.updateOne(Message, query, updateBody);
      if (!updatedMessage){
        return res.recordNotFound();
      }
      return res.success({ data:updatedMessage });
...
const deleteMessage = async (req,res) => {
    try { 
      ...
      const query = { _id:req.params.id };
      const deletedMessage = await dbService.deleteOne(Message, query);
      if (!deletedMessage){
        return res.recordNotFound();
      }
 ...
 */
const deleteManyMessage = async (req, res) => {
    try {
      ...
      const deletedMessage = await dbService.deleteMany(Message,query);
      if (!deletedMessage){
        return res.recordNotFound();
      }
      return res.success({ data :{ count :deletedMessage } });
    } catch (error){
      return res.internalServerError({ message:error.message }); 
    }
  };
 ...
 */
const softDeleteManyMessage = async (req,res) => {
  try {
    let ids = req.body.ids;
    if (!ids || !Array.isArray(ids) || ids.length < 1) {
      return res.badRequest();
    ...
    let updatedMessage = await dbService.updateMany(Message,query, updateBody);
    if (!updatedMessage) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedMessage } });
