...
const softDeleteMeeting = async (req,res) => {
    try {
      ...
      let updatedMeeting = await dbService.updateOne(Meeting, query, updateBody);
      if (!updatedMeeting){
        return res.recordNotFound();
      }
      return res.success({ data:updatedMeeting });
...
const deleteMeeting = async (req,res) => {
    try { 
      ...
      const query = { _id:req.params.id };
      const deletedMeeting = await dbService.deleteOne(Meeting, query);
      if (!deletedMeeting){
        return res.recordNotFound();
      }
...
const deleteManyMeeting = async (req, res) => {
    try {
      ...
      const deletedMeeting = await dbService.deleteMany(Meeting,query);
      if (!deletedMeeting){
        return res.recordNotFound();
      }
      return res.success({ data :{ count :deletedMeeting } });
    } catch (error){
      return res.internalServerError({ message:error.message }); 
    }
  };
...
const softDeleteManyMeeting = async (req,res) => {
  try {
    ...
    let updatedMeeting = await dbService.updateMany(Meeting,query, updateBody);
    if (!updatedMeeting) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedMeeting } });
