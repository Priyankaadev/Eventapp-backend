...
const reviewSchemaKey = require('../../../utils/validation/reviewValidation');
...
/**
 * @description : create document of Review in mongodb collection.
 * @param {Object} req : request including body for creating document.
 * @param {Object} res : response of created document
 * @return {Object} : created Review. {status, message, data}
 */ 
const addReview = async (req, res) => {
  try {
    ...
    let validateRequest = validation.validateParamsWithJoi(
      dataToCreate,
      reviewSchemaKey.schemaKeys);
    if (!validateRequest.isValid) {
      return res.validationError({ message : `Invalid values in parameters, ${validateRequest.message}` });
    }
    dataToCreate.addedBy = req.user.id;
    dataToCreate = new Review(dataToCreate);
    let createdReview = await dbService.create(Review,dataToCreate);
    return res.success({ data : createdReview });
  } catch (error) {
 ...
 */
const bulkInsertReview = async (req,res)=>{
    try {
      ...
      let createdReviews = await dbService.create(Review,dataToCreate);
      createdReviews = { count: createdReviews ? createdReviews.length : 0 };
      return res.success({ data:{ count:createdReviews.count || 0 } });
    } catch (error){
      return res.internalServerError({ message:error.message });
    }
  };
...
const findAllReview = async (req,res) => {
    try {
      ...
      let validateRequest = validation.validateFilterWithJoi(
        req.body,
        reviewSchemaKey.findFilterKeys,
        Review.schema.obj
      );
      if (!validateRequest.isValid) {
      ...
      let foundReviews = await dbService.paginate( Review,query,options);
      if (!foundReviews || !foundReviews.data || !foundReviews.data.length){
...
const getReview = async (req,res) => {
    try {
      ...
      let options = {};
      let foundReview = await dbService.findOne(Review,query, options);
      if (!foundReview){
        return res.recordNotFound();
      }
      return res.success({ data :foundReview });
    }
    catch (error){
      return res.internalServerError({ message:error.message });
    }
...
const getReviewCount = async (req,res) => {
    try {
      ...
      let countedReview = await dbService.count(Review,where);
      return res.success({ data : { count: countedReview } });
    } catch (error){
      return res.internalServerError({ message:error.message });
    }
  };
...
const updateReview = async (req,res) => {
    ...
    // let {name, city, date, status} = req.body;
      ...
      let validateRequest = validation.validateParamsWithJoi(
        dataToUpdate,
        reviewSchemaKey.updateSchemaKeys
      );
      if (!validateRequest.isValid) {
        return res.validationError({ message : `Invalid values in parameters, ${validateRequest.message}` });
      }
      const query = { _id:req.params.id };
      let updatedReview = await dbService.updateOne(Review,query,dataToUpdate);
      if (!updatedReview){
        return res.recordNotFound();
      }
      return res.success({ data :updatedReview });
    } catch (error){
      return res.internalServerError({ message:error.message });
    }
  };
 ...
 */
const bulkUpdateReview = async (req,res)=>{
    try {
      ...
      let updatedReview = await dbService.updateMany(Review,filter,dataToUpdate);
      if (!updatedReview){
        return res.recordNotFound();
      }
      return res.success({ data :{ count : updatedReview } });
    } catch (error){
      return res.internalServerError({ message:error.message }); 
    }
  };
...
const softDeleteReview = async (req,res) => {
    try {
      ...
      let updatedReview = await dbService.updateOne(Review, query, updateBody);
      if (!updatedReview){
        return res.recordNotFound();
      }
      return res.success({ data:updatedReview });
...
const deleteReview = async (req,res) => {
    try { 
      ...
      const query = { _id:req.params.id };
      const deletedReview = await dbService.deleteOne(Review, query);
      if (!deletedReview){
        return res.recordNotFound();
      }
      return res.success({ data :deletedReview});
...
const deleteManyReview = async (req, res) => {
    try {
      ...
      const deletedReview = await dbService.deleteMany(Review,query);
      if (!deletedReview){
        return res.recordNotFound();
      }
      return res.success({ data :{ count :deletedReview } });
    } catch (error){
      return res.internalServerError({ message:error.message }); 
    }
  };
 ...
 */
const softDeleteManyReview = async (req,res) => {
  try {
    let ids = req.body.ids;
    if (!ids || !Array.isArray(ids) || ids.length < 1) {
      return res.badRequest();
    ...
    let updatedReview = await dbService.updateMany(Review,query, updateBody);
    if (!updatedReview) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedReview } });
